/* Created By Feifei Pan, Fangyan Xu and Chengjian Zheng */
...
// Scan from the "start" to find the next available memframe.
// Cursor is only specified if the cursor has reached the end 
// of the frame and returned to the 0 position (specified in the 
// "nextFit_place" method).
int MMS::findNextAvailable(int start, int cursor = NUM_MEM_FRAME+1)
{
	int pos = start;
	while (1)
	{
		if (pos >= cursor) // Signal nextFit_place() methd a full scan has completed
			return -2;
		if (pos >= NUM_MEM_FRAME-1) // Scan has reached the end frame
			return -1;
		if (mem[pos] == '.')
			break;
		else
...
// Scan from the "start" to find the next occupied memframe
...
int MMS::defragmentation()
{
	...
	while (1)
	{
		...
		if (occupiedEnd < 0) // Already at the end frame
		{
			if (mem[NUM_MEM_FRAME-1] == '.') // if the last frame is empty
				occupiedEnd = NUM_MEM_FRAME-2;
			else // if the last frame is occupied
...
void MMS::contiguous(){
	...
	while(simuTimer >= 0 && !all_finished )
	{//simulation starts
		checkEnd(simuTimer); 
...
// Check the proceses that may arrive in the current time step
		...
		while(jobit != it->listOfJobs.end()){
			if(jobit->first + defragmentationTime <= t){
				...
				else if (method == string("Best-Fit"))
				{
					...
					else // No enough room to place the process
					{
						cout << "time "<< simuTimer <<"ms: Cannot place process " 
						 << it->procID << " -- skipped!" << endl;
						 if(it->listOfJobs.size() > 1){
...
// Check the processes that may be removed in the current time step
...
int MMS::nextFit_place(char procID, int num)
{
	...
	while (1)
	{
		...
		if (start == -2) // if a full scan is finished, but no placing done, then perform defragmentation
		{
...
{
	// Read the input for memory management
	...
	// Parse virtual memeory input file, save all page numbers 
	ifstream vm(argv[2]);
	int page_num;
	string page;
	vector<int> pages;
